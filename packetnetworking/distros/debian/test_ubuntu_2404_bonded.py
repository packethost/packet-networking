...
@pytest.fixture
def ubuntu_2404_bonded_network(generic_debian_bonded_network):
    def _builder(**kwargs):
        return generic_debian_bonded_network("ubuntu", "24.04", **kwargs)

    return _builder


def test_ubuntu_2404_public_bonded_task_etc_network_interfaces(
    ubuntu_2404_bonded_network,
):
    """Validates /etc/network/interfaces for a public bond"""

    builder = ubuntu_2404_bonded_network(public=True)
...
def test_ubuntu_2404_private_bonded_task_etc_network_interfaces(
    ubuntu_2404_bonded_network,
):
    """
    ...
    """
    builder = ubuntu_2404_bonded_network(public=False)
    tasks = builder.render()
    result = dedent(
        """\
        auto lo
...
def test_ubuntu_2404_public_bonded_task_etc_network_interfaces_with_custom_private_ip_space(
    ubuntu_2404_bonded_network,
):
    """Validates /etc/network/interfaces for a public bond"""
    subnets = {"private_subnets": ["192.168.5.0/24", "172.16.0.0/12"]}
    builder = ubuntu_2404_bonded_network(public=True, metadata=subnets)
    tasks = builder.render()
    result = dedent(
        """\
        auto lo
...
def test_ubuntu_2404_private_bonded_task_etc_network_interfaces_with_custom_private_ip_space(
    ubuntu_2404_bonded_network,
):
    """
    When no public ip is assigned, we should see the private ip details in the
    ...
    """
    subnets = {"private_subnets": ["192.168.5.0/24", "172.16.0.0/12"]}
    builder = ubuntu_2404_bonded_network(public=False, metadata=subnets)
    tasks = builder.render()
    result = dedent(
        """\
        auto lo
...
def test_ubuntu_2404_task_etc_modules(ubuntu_2404_bonded_network):
...
def test_ubuntu_2404_etc_systemd_resolved_configured(ubuntu_2404_bonded_network, fake):
    """
    Validates /etc/systemd/resolved.conf is configured correctly
    """
    builder = ubuntu_2404_bonded_network()
    resolver1 = fake.ipv4()
    resolver2 = fake.ipv4()
...
def test_ubuntu_2404_etc_hostname_configured(ubuntu_2404_bonded_network):
    """
...
def test_ubuntu_2404_etc_hosts_configured(ubuntu_2404_bonded_network):
    """
...
def test_ubuntu_2404_persistent_interface_names(ubuntu_2404_bonded_network):
    """
