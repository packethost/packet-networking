...
def test_opensuseproject_private_only_throws_error(opensuseproject_individual_network):
    """
    Verifies a jinja2 UndefinedError is thrown when providing only
    private ip information
    """
    builder = opensuseproject_individual_network(public=False)
...
def test_opensuseproject_public_individual_task_etc_sysconfig_network_ifcfg_enp0(
    ...
    builder = opensuseproject_individual_network(public=True)
...
def test_opensuseproject_public_route_task_etc_sysconfig_network_routes_with_private_subnet_routes(
    ...
    builder = opensuseproject_individual_network(public=True, metadata=subnets)
    tasks = builder.render()
    result = dedent(
        """\
        default     {ipv4pub.gateway}
...
def test_opensuseproject_etc_resolvers_configured(
    opensuseproject_individual_network, fake
):
    """
    Validates /etc/resolv.conf is configured correctly
    """
    builder = opensuseproject_individual_network()
    resolver1 = fake.ipv4()
    resolver2 = fake.ipv4()
...
def test_opensuseproject_etc_hostname_configured(opensuseproject_individual_network):
    """
...
def test_opensuseproject_etc_hosts_configured(opensuseproject_individual_network):
    """
