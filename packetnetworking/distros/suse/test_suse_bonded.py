...
@pytest.fixture
def suse_bonded_network(generic_suse_bonded_network):
    def _builder(**kwargs):
...
def test_suse_private_only_throws_error(suse_bonded_network):
    """
    Verifies a jinja2 UndefinedError is thrown when providing only
    private ip information
    """
    builder = suse_bonded_network(public=False)
...
def test_suse_public_route_task_etc_sysconfig_network_routes(suse_bonded_network):
    """
...
def test_suse_public_route_task_etc_sysconfig_network_routes_with_private_subnet_routes(
    ...
    builder = suse_bonded_network(public=True, metadata=subnets)
    tasks = builder.render()
    result = dedent(
        """\
        default     {ipv4pub.gateway}
...
def test_suse_public_task_etc_sysconfig_network_ifcfg_enp0(suse_bonded_network):
    """
...
def test_suse_public_task_etc_sysconfig_network_ifcfg_enp1(suse_bonded_network):
    """
    For each interface, we should see the corresponding ifcfg file
    located at /etc/sysconfig/network/ifcfg-*
    """
...
def test_suse_etc_resolvers_configured(suse_bonded_network, fake):
    """
...
def test_suse_etc_hostname_configured(suse_bonded_network):
    """
...
def test_suse_etc_hosts_configured(suse_bonded_network):
    """
