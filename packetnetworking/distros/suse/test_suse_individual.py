...
@pytest.fixture
def suse_individual_network(generic_suse_individual_network):
    def _builder(**kwargs):
        return generic_suse_individual_network("suse", "7", **kwargs)

    return _builder


def test_suse_private_only_throws_error(suse_individual_network):
    """
    Verifies a jinja2 UndefinedError is thrown when providing only
    private ip information
    """
    builder = suse_individual_network(public=False)
...
def test_suse_public_route_task_etc_sysconfig_network_routes(suse_individual_network):
    """
...
def test_suse_public_route_task_etc_sysconfig_network_routes_with_private_subnet_routes(
    ...
    builder = suse_individual_network(public=True, metadata=subnets)
    tasks = builder.render()
    result = dedent(
        """\
        default     {ipv4pub.gateway}
...
def test_suse_public_task_etc_sysconfig_network_ifcfg_enp1(suse_individual_network):
    """
...
def test_suse_etc_resolvers_configured(suse_individual_network, fake):
    """
    Validates /etc/resolv.conf is configured correctly
    """
    builder = suse_individual_network()
    resolver1 = fake.ipv4()
    resolver2 = fake.ipv4()
...
def test_suse_etc_hostname_configured(suse_individual_network):
    """
...
def test_suse_etc_hosts_configured(suse_individual_network):
    """
